...
def generate_prescription_pdf(prescription, request=None) -> bool:
    ...
    try:
        ...
        brand_s   = S("brand",   fontSize=8,  textColor=TEAL,  fontName="Helvetica-Bold", spaceAfter=1)
        h1_s      = S("h1",      fontSize=14, textColor=BLACK, fontName="Helvetica-Bold", spaceAfter=2)
        ...
        val_s     = S("val",     fontSize=10, textColor=BLACK, fontName="Helvetica-Bold")
        sec_s     = S("sec",     fontSize=8,  textColor=TEAL,  fontName="Helvetica-Bold", spaceAfter=4)
        body_s    = S("body",    fontSize=10, textColor=BLACK)
        small_s   = S("small",   fontSize=8,  textColor=GRAY)
        right_s   = S("right",   fontSize=9,  textColor=GRAY,  alignment=TA_RIGHT)
        rxnum_s   = S("rxnum",   fontSize=13, textColor=TEAL,  fontName="Helvetica-Bold", alignment=TA_RIGHT)
        ...
        # ── Header ───────────────────────────────────────────────────────────
        header_left = (
            f"<b>CareConnect · E-Prescription</b><br/>"
            f"<font size='14'><b>{doctor_name}</b></font><br/>"
            f"{specialty + (f' · {clinic_name}' if clinic_name else '')}"
        )
        header_right = (
            "Prescription No.<br/>"
            f"<font size='13' color='#0f766e'><b>RX-{prescription.pk:06d}</b></font><br/>"
            f"Date: {apt_date or prescription.date.strftime('%B %d, %Y')}"
        )
        header_data = [[
            Paragraph(header_left, sub_s),
            Paragraph(header_right, right_s),
        ]]
        ...
        pt_tbl = Table([
            [Paragraph(f"<b>Patient Name</b><br/>{patient_name or '—'}", body_s),
             Paragraph(f"<b>Age / Sex</b><br/>{age_sex or '—'}", body_s)],
            [Paragraph(f"<b>Address</b><br/>{patient_addr}", body_s),
             Paragraph(f"<b>Consultation Date</b><br/>{apt_date or '—'}", body_s)],
        ...
        med_rows = [[Paragraph("#", label_s), Paragraph("Medicine", label_s),
                     Paragraph("Sig / Directions", label_s), Paragraph("Qty", label_s)]]
        for i, med in enumerate(clean_meds, 1):
            ...
            med_html = f"<b>{display}</b>"
            if sub_info:
                med_html += f"<br/><font size='8' color='#6b7280'>{sub_info}</font>"

            qty_html = f"<b>{qty}</b>"
            if refills:
                qty_html += f"<br/><font size='8' color='#6b7280'>Refills: {refills}</font>"

            med_rows.append([
                Paragraph(str(i), body_s),
                Paragraph(med_html, body_s),
                Paragraph(sig_full.replace("\n", "<br/>"), body_s),
                Paragraph(qty_html, body_s),
            ])
        ...
        notes_tbl = Table([
            [Paragraph(f"<b>DIAGNOSIS</b><br/>{prescription.diagnosis or '—'}", body_s),
             Paragraph(f"<b>FOLLOW-UP DATE</b><br/>{follow_up}", body_s)],
            [Paragraph(f"<b>REMARKS / INSTRUCTIONS</b><br/>{remarks}", body_s),
             Paragraph(f"<b>VALID UNTIL</b><br/>{prescription.valid_until.strftime('%B %d, %Y')}", body_s)],
        ], colWidths=[82*mm, 82*mm])
        ...
        # ── Signature ────────────────────────────────────────────────────────
        creds = " · ".join(filter(None, [
            f"PRC: {prc}" if prc else "",
            f"PTR: {ptr}" if ptr else "",
        ]))
        sig_left = (
            f"<b>{doctor_name}</b><br/>"
            f"<font size='8' color='#6b7280'>{creds or 'Digitally signed via CareConnect'}</font>"
        )
        sig_tbl = Table([[Paragraph(sig_left, body_s),
                          Paragraph("Digitally verified prescription<br/>issued via CareConnect", footer_s)]],
        ...
        story.append(Paragraph(
            "This electronic prescription is valid when verified by the prescribing physician.",
            footer_s
        ))
        ...
        logger.error("PDF generation error for prescription #%s: %s", prescription.pk, exc)
        return False
