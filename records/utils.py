...
def generate_certificate_pdf(cert, request=None) -> bool:
    ...
    try:
        ...
        # ── Header ────────────────────────────────────────────────────────────
        header_left = (
            f"<b>CareConnect · Medical Certificate</b><br/>"
            f"<font size='14'><b>{doctor_name}</b></font><br/>"
            f"{specialty + (f' · {clinic_name}' if clinic_name else '')}"
        )
        header_right = (
            "Certificate No.<br/>"
            f"<font size='13' color='#0f766e'><b>CERT-{cert.pk:06d}</b></font><br/>"
            f"Date: {apt_date}"
        )
        header_tbl = Table(
            [[Paragraph(header_left, sub_s), Paragraph(header_right, right_s)]],
        ...
        pt_tbl = Table([
            ...
            [Paragraph(f"<b>Address</b><br/>{patient_addr}", body_s),
             Paragraph(f"<b>Consultation Date</b><br/>{apt_date}", body_s)],
        ...
        sig_tbl = Table(
            [[Paragraph(sig_left, body_s),
              Paragraph("Digitally verified certificate<br/>issued via CareConnect", footer_s)]],
        ...
        story.append(Paragraph(
            "This medical certificate is valid when verified by the issuing physician.",
            footer_s,
        ))

        doc.build(story)
        filename = f"certificate_{cert.pk}.pdf"
        cert.pdf_file.save(filename, ContentFile(buf.getvalue()), save=True)
